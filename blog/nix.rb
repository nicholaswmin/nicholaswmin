...
module FS
  ...
  def self.write(dest:, force: false)
    lambda do |(path, data)|
      ...
      Log.warn("write #{pathname}, skipped. exists") if
        File.exist?(pathname) && !force
...
opts = OptionParser.new('docs: https://github.com/nicholaswmin/nix') do |o|
  ...
  o.on('-s', '--serve', 'serve site at port', Integer)
...
spawn "ruby -run -e httpd . -p #{params[:serve] ||= 0}" if params[:serve]
