...
def init dir: 'blog'
  [
    # config
    [File.basename(__FILE__), File.read(__FILE__)],
    # config
    ...
    # pages
    ['pages/index.md', "hello world"], ['pages/about.md', "¦## About¦¦¦ This is a sample `Page`. it's like a post but not quite. ¦¦It's is written in `Markdown` but doesn't support code syntax highlighting. A blockquote follows:¦¦> nor is it included in any Post lists.¦¦You can add as many as you want within this folder.¦¦Merci¦"], ['pages/index.md', "hello world :)" ],
    # Felix the housecat 
  ...
  .map{| e | [File.join(dir, e[0]),e[1].gsub('¦', "\n").gsub('┊', '"').strip] }
