...
def init
  [
    ...
    ['pages/about.md', "¦## About¦¦¦ This is a sample `Page`. it's like a post but not quite. ¦¦It's is written in `Markdown` but doesn't support code syntax highlighting. A blockquote follows:¦¦> nor is it included in any Post lists.¦¦You can add as many as you want within this folder.¦¦Merci¦"],
    ['pages/index.md', "hello world :)" ],
    ...
    ['public/styles/highlight', Rouge::Themes::Github.mode(:light).render(scope: '.highlight').gsub("\n", '¦').gsub('"', '┊')]
  ]
  .map{ | e | [Pathname.new(e[0]), e[1].gsub('¦', "\n").gsub('┊', '"').strip] }
